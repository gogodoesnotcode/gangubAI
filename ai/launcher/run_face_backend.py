...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--backend-mode",
        choices=("wakeword", "ptt"),
        default="wakeword",
        help="Backend input mode. Use wakeword to stay on frontend display.",
    )
    parser.add_argument(
        "--backend-log",
...
def _build_backend_cmd(mode: str, no_tts: bool) -> list[str]:
    cmd = ["python3", "-m", "ai.voice.app"]
...
def main() -> int:
    ...
    backend_cmd = _build_backend_cmd(args.backend_mode, args.no_tts)
